...
from scripts.phase3_collect_fotmob_identity import _sofascore_in_cooldown,SOFASCORE_COOLDOWN_SECONDS
...
class SourceHealthTests(unittest.TestCase):
    def test_recent_block_is_cooled_down(self):
        ...
        blocked,age=_sofascore_in_cooldown(state,now)
    ...
    def test_old_block_retries(self):
        ...
        blocked,_=_sofascore_in_cooldown(state,now)
        self.assertFalse(blocked)

    def test_missing_or_bad_timestamp_does_not_permanently_block(self):
        now=datetime(2026,9,21,16,0,tzinfo=timezone.utc)
        self.assertFalse(_sofascore_in_cooldown({},now)[0])
        self.assertFalse(_sofascore_in_cooldown({"SOFASCORE":{"blocked_at":"bad"}},now)[0])
