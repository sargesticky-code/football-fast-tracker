...
score as (
 select
   hkjc_event_id,
   updated_at_source live_score_captured_at,
   source_updated_at live_score_source_updated_at,
   live_score,
   home_score,
   away_score,
   minute live_minute,
   match_status live_score_status,
   source live_score_source,
   match_confidence live_score_confidence,
   home_corners,
   away_corners,
   total_corners
 from public.live_score_current
 where updated_at_source >= now()-interval '10 minutes'
   and upper(coalesce(match_status,'')) not in ('FULLTIME','FINISHED','FT','ENDED','MATCHENDED','AET','PEN','CANCELLED','CANCELED','VOID','ABANDONED')
