...
begin
  with agg as (
    ...
    where source_match_id is not null
      ...
      and captured_at_hkt >= now()-interval '4 hours'
  ...
  stable as (
    ...
    where a.observations >= 3 and a.min_conf >= 0.85 and v.id_variants=1
  ),
  ins as (
    ...
    where public.phase3_live_identity_map.source_match_id=excluded.source_match_id
      and (
        excluded.last_verified_at > public.phase3_live_identity_map.last_verified_at
        or excluded.evidence_count > public.phase3_live_identity_map.evidence_count
      )
    returning 1
  )
  ...
  src as (
    select a.*,v.id_variants,
      case when a.observations>=3 and a.min_conf>=0.85 and v.id_variants=1
           then 'PROMOTABLE' else 'CANDIDATE' end candidate_state
    from agg a join variants v using(hkjc_event_id,source)
  ),
  ins as (
    ...
    where excluded.last_seen_at > public.phase3_live_identity_candidate.last_seen_at
       or excluded.evidence_count > public.phase3_live_identity_candidate.evidence_count
    returning 1
  )
  select count(*) into v_candidates from ins;

  select eligible_rows,verified_rows into v_eligible,v_mapped
  ...
  values(
    ...
    case when v_eligible=0 or v_mapped=v_eligible then 'OK' else 'WARN' end,
    concat(v_mapped,'/',v_eligible),
    'Incremental Phase 3 live identity registry refresh.',
    now(),
    jsonb_build_object(
      'eligible',v_eligible,'mapped',v_mapped,
      'map_writes',v_promoted,'candidate_writes',v_candidates,
      'lookback','4h','cadence','2m'
    )
  )
...
select cron.alter_job(
  (select jobid from cron.job where jobname='phase3-identity-registry-1min' limit 1),
  schedule := '*/2 * * * *'
);
