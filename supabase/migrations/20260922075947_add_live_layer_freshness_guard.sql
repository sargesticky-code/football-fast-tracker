...
begin
  ...
  audit as (
    select
      l.hkjc_event_id,
      case when s.updated_at_source is null or s.updated_at_source < v_now - interval '3 minutes' then true else false end as score_bad,
      case when st.captured_at_hkt is null or st.captured_at_hkt < v_now - interval '10 minutes' then true else false end as stats_bad,
      case
      ...
      s.updated_at_source as score_at,
      st.captured_at_hkt as stats_at,
      sh.captured_at_hkt as shadow_at,
      s.minute
    from live l
  ...
  into v_live_count,v_score_bad,v_stats_bad,v_shadow_bad,v_raw
  from audit;
  ...
  if v_live_count = 0 then
    ...
    v_status := 'WARN';
    v_notes := format('Core live fresh; detail lag stats=%s shadow=%s across %s live matches.',v_stats_bad,v_shadow_bad,v_live_count);
  else
    v_status := 'OK';
  ...
  values(
    ...
    concat('live=',v_live_count,' score_bad=',v_score_bad,' stats_bad=',v_stats_bad,' shadow_bad=',v_shadow_bad),
    v_notes,v_now,
    jsonb_build_object(
      'live_count',v_live_count,
      'score_bad',v_score_bad,
      'stats_bad',v_stats_bad,
      'shadow_bad',v_shadow_bad,
      'matches',v_raw
    )
  )
  ...
  return jsonb_build_object(
    'status',v_status,'live_count',v_live_count,
    'score_bad',v_score_bad,'stats_bad',v_stats_bad,'shadow_bad',v_shadow_bad
  );
end;
$$;

revoke all on function public.ft_refresh_live_layer_guard() from public, anon, authenticated;
grant execute on function public.ft_refresh_live_layer_guard() to service_role;

do $$
declare j record;
begin
  for j in select jobid from cron.job where jobname='live-layer-guard-1min'
  loop
...
select cron.schedule(
  'live-layer-guard-1min',
  '* * * * *',
  $cron$select public.ft_refresh_live_layer_guard();$cron$
);
