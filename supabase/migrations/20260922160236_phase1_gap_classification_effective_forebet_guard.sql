...
case
 when upper(q.source)='FOREBET' and q.reason='current_model_missing_after_reconcile' and exists (select 1 from public.forebet_effective_v e where e.hkjc_event_id=q.last_event_id and e.forebet_status='CURRENT') then 'RESOLVED_CURRENT_MODEL_PRESENT'
 ...
 else 'REVIEW_REQUIRED' end gap_class,
exists(select 1 from public.team_name_master m where m.team_key=q.team_key and m.status='VERIFIED') has_verified_static_identity,
fq.queue_reason provider_surface_reason
