"""Run the complete Phase-2 Layer-1 pipeline in one executable command.
Keeps orchestration outside production and returns non-zero on stale/empty capture.
"""
...
for i,cmd in enumerate(STEPS,1):
 print(f"PHASE2_LAYER1 step={i}/{len(STEPS)} cmd={' '.join(cmd)}",flush=True)
