...
def post(table, rows, conflict, batch=250, merge=True):
...
def sync_supplement():
    rows=map_rows("forebet_supplement_current.csv", lambda r: None if blank(r.get("hkjc_event_id")) else {
      "hkjc_event_id":text(r.get("hkjc_event_id")),"fetched_at":timestamp(r.get("fetched_at_hkt")),
      "kickoff_hkt":timestamp(r.get("kickoff_hkt")),"hkjc_league":text(r.get("hkjc_league")),
...
def sync_archive():
    rows=map_rows("forebet_archive.csv", lambda r: None if blank(r.get("hkjc_event_id")) or blank(r.get("captured_at_hkt")) else {
      "hkjc_event_id":text(r.get("hkjc_event_id")),"captured_at":timestamp(r.get("captured_at_hkt")),
      ...
      "hkjc_away_zh":text(r.get("hkjc_away_zh")),"hkjc_kickoff_hkt":timestamp(r.get("hkjc_kickoff_hkt")),
      "prob_home":num(r.get("prob_home")),"prob_draw":num(r.get("prob_draw")),"prob_away":num(r.get("prob_away")),
      "prediction_1x2":text(r.get("prediction_1x2")),"predicted_score":text(r.get("predicted_score")),
      "avg_goals":num(r.get("avg_goals")),"power_home":num(r.get("power_home")),"power_away":num(r.get("power_away")),
      "power_source":text(r.get("power_source")),"power_updated":timestamp(r.get("power_updated")),
...
def sync_availability():
    rows=map_rows("forebet_availability.csv", lambda r: None if blank(r.get("hkjc_event_id")) else {
      "hkjc_event_id":text(r.get("hkjc_event_id")),"checked_at":timestamp(r.get("checked_at_hkt")),
...
def sync_evaluation():
    rows=map_rows("evaluation_summary.csv", lambda r: None if blank(r.get("model")) else {
      "model":text(r.get("model")),"as_of_hkt":timestamp(r.get("as_of_hkt")),
...
def sync_live_score():
    ...
    for r in src:
      ...
      rows.append({
        "hkjc_event_id":event,"updated_at_source":timestamp(r.get("updatedAt")),"kickoff_hkt":text(r.get("kickoff_hkt")),
        ...
        "match_confidence":num(r.get("match_confidence")),"source_updated_at":timestamp(r.get("source_updated_at")),
