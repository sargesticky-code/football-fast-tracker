...
BASE='https://www.fotmob.com/api/data/teams'; WORKERS=8
...
def members(payload):
 ...
 if not out:
  ...
  walk(root)
 return out
def fetch_team(item,now):
 hid,r=item;tid=r['external_team_id'];url=f'{BASE}?id={tid}'
 try:
  ...
  if q.status_code!=200:return [],{'hkjc_team_id':hid,'team':r['hkjc_name_en'],'class':'SOURCE_ERROR','detail':str(q.status_code)}
  ...
  if not ms:
   ...
   explicit_roster=isinstance(sq,dict) and 'squad' in sq
   provider_empty=explicit_roster and (groups is None or (isinstance(groups,list) and group_count==0 and member_count==0))
   klass='PROVIDER_ROSTER_EMPTY' if provider_empty else 'SCHEMA_UNRESOLVED'
   return [],{'hkjc_team_id':hid,'team':r['hkjc_name_en'],'class':klass,'detail':json.dumps({'squad_keys':sorted(sq.keys()) if isinstance(sq,dict) else [],'squad_value_type':type(groups).__name__,'group_count':group_count,'member_count':member_count},separators=(',',':'))}
  rows=[]
  for m in ms:
  ...
  return rows,None
 except Exception as e:return [],{'hkjc_team_id':hid,'team':r['hkjc_name_en'],'class':'SOURCE_ERROR','detail':type(e).__name__+': '+str(e)[:120]}
def main():
 ...
 with ThreadPoolExecutor(max_workers=min(WORKERS,max(1,len(teams)))) as ex:
  futs=[ex.submit(fetch_team,item,now) for item in sorted(teams.items())]
  for fut in as_completed(futs):
   rs,fail=fut.result()
   if rs:covered+=1;rows.extend(rs)
   if fail:failures.append(fail)
 rows.sort(key=lambda x:(x['hkjc_team_id'],x['canonical_name'],x['player_id']));failures.sort(key=lambda x:x['hkjc_team_id'])
 ...
 health={'status':'OK' if teams else 'NO_TARGETS','target_confirmed_fotmob_teams':len(teams),'teams_with_squad':covered,'team_coverage_pct':pct,'players':n,'position_coverage_pct':round(100*pos/n,1) if n else 0,'dob_coverage_pct':round(100*dob/n,1) if n else 0,'age_coverage_pct':round(100*age/n,1) if n else 0,'nationality_coverage_pct':round(100*nat/n,1) if n else 0,'failure_classes':classes,'residual_all_classified':residual_all_classified,'layer2_exit':layer2_exit,'failures':failures,'workers':WORKERS,'fetched_at':now}
